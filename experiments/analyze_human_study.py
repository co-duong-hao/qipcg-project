...
def paired_permutation_pvalue(a: np.ndarray, b: np.ndarray, permutations: int, seed: int) -> tuple[float, float]:
    ...
    for _ in range(permutations):
        ...
        permuted = abs(float(np.mean(diffs * signs)))
        if permuted >= abs_observed - 1e-12:
            count += 1
    p_value = (count + 1) / float(permutations + 1)
    return observed, p_value


def holm_adjust(p_values: list[float]) -> list[float]:
    indexed = [(i, p) for i, p in enumerate(p_values)]
    valid = sorted([(i, p) for i, p in indexed if not pd.isna(p)], key=lambda item: item[1])
...
def clean_responses(responses: pd.DataFrame, answer_key: pd.DataFrame) -> pd.DataFrame:
    ...
    merged = responses.merge(
        answer_key[["stimulus_id", "dataset", "method"]],
        on="stimulus_id",
        how="left",
        validate="many_to_one",
    )
    missing = merged["method"].isna().sum()
    if missing:
    ...
    for col in RATING_COLUMNS:
        ...
        if bad.any():
            raise SystemExit(f"Column {col} contains ratings outside the 1--5 range")
    return merged
...
def pairwise_tests(cleaned: pd.DataFrame, permutations: int, seed: int) -> pd.DataFrame:
    participant_means = (
        cleaned.groupby(["participant_id", "dataset", "method"], as_index=False)[RATING_COLUMNS]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze blinded mini human-study responses.")
    parser.add_argument("--responses", type=Path, required=True)
    parser.add_argument("--answer-key", type=Path, required=True)
    parser.add_argument("--out-dir", type=Path, default=Path("human_study/results_seed2026"))
    ...
    cleaned.to_csv(args.out_dir / "human_study_cleaned_responses.csv", index=False)
    summary.to_csv(args.out_dir / "human_study_summary.csv", index=False)
    tests.to_csv(args.out_dir / "human_study_pairwise_tests.csv", index=False)
